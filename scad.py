...
def scad(input_, theta_, gamma):
    out = torch.where( (torch.abs(input_) > gamma*theta_), input_, input_ )
    out = torch.where( (torch.abs(input_) > 2 * theta_) & (torch.abs(input_) <= gamma*theta_), (((gamma-1)*input_ - torch.sign(input_)*gamma*theta_)/(gamma-2)), out )
    out = torch.where( (torch.abs(input_) <= 2 * theta_), torch.sign(input_) * torch.max( torch.abs(input_) - theta_, torch.zeros_like(input_)), out )
    return out
...
def forward(y,x_test, thr_, gam, d, B, W, numIter):
    ...
    for iter in range(numIter):
      xold = x_k.clone()
      x_k  = scad(np.matmul(B,y) + np.matmul(W,d), thr_, gam)
...
space = {'lambda': hp.uniform('lambda', 0,1),'gamma': hp.uniform('gamma', 1,100)}  ####'lambda': hp.uniform('lambda', 0,1),
...
# f = open("scad/logs/scad measurement {} sparsity {}.txt".format(m, args.sparsity), "w")
# # f.write('Lamda is {}\n'.format( lam))
# # f.write('gamma is {}\n'.format( gam))
