...
def forward(y, x_test, scale, w_init, gama, d, B, W, numIter, p, J):
    x         = []
    x.append(d)
    # RSNR_iter = []
    ...
    t         = 1
    x_k       = d.clone()
    w_k       = tf.ones(d.shape)*w_init

    for iter in range(numIter):
      xold = tf.identity(x_k)
      x_k  = GST(np.matmul(B,y) + np.matmul(W,d), w_k*(1/scale), p, J)
      
      t0 = t
      t  = (1. + np.sqrt(1. + 4. * t ** 2)) / 2.0
      d  = x_k + ((t0 - 1.0) / t) * (x_k - xold)
      w_k     = tf.maximum(w_k - (tf.abs(d)**p)*(1/gama),0.0)
      x.append(d)
...
def objective(params):
    ...
    gama  = params['gamma'] #46.081#params['gamma']  
    ...
    x_est = forward(y_train, x_train, scale, w_init, gama, d, B, W, numIter, p, J)
    x_est = x_est[-1].numpy()
...
space = {'p': hp.uniform('p',0.1,1.1),'w_init':hp.uniform('w_init',0,0.1),'gamma': hp.uniform('gamma', 1,100000)}  ####
...
# x_est = forward(y_test, x_test, thr, w_init, gama, d, B, W, numIter, p, J)
# x_est = x_est[-1].numpy()
