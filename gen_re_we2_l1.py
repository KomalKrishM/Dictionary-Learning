...
parser = ArgumentParser(description='Generalized_reweighted_l1')
...
parser.add_argument('--m', type=int, default=275, help='measurement_vector_length')
...
def forward(y,x_test, thr_, d, B, W, numIter, tau, eps):
    x           = []
    x.append(d)
    # RSNR_iter = []
    weights     = np.ones(d.shape)*0.03
    ...
    for iter in range(numIter):
      xold    = x_k.clone()
      x_k     = soft_thr(np.matmul(B,y) + np.matmul(W,d), weights*thr_)
      t0      = t
      t       = (1. + np.sqrt(1. + 4. * t ** 2)) / 2.0
      d       = x_k + ((t0 - 1.0) / t) * (x_k - xold)
      weights = tau*k/(np.abs(d) + eps)**(1/(k+1))
      x.append(d)
...
# x_est = forward(y_train, x_train, thr, d, B, W, numIter, tau, eps)
# x_est = x_est[-1].numpy()
...
# space = {
#         'TAU': hp.uniform('TAU', 0,1), 'EPS': hp.uniform('EPS', 0,1)} ######  'lambda': hp.uniform('lambda', 0.001,1),
...
print('Testing: my gen_l1 avg SNR is ', np.mean(RSNR_test))
print('Testing: my gen_l1 std deviation in SNR is ', np.std(RSNR_test))
print('Testing: my gen_l1 peak SNR is ', np.max(RSNR_test))

print('Testing: my gen_l1 avg PES is ', PES_mean)
print('Testing: my gen_l1 std deviation in PES is ', PES_std)
print('-'*40)
...
f = open("Gen_Re_We_l1/logs2/Gen2_L1 measurement {} sparsity {}.txt".format(m, args.sparsity), "w")
# f.write('Lamda is {}\n'.format( lam))
# f.write('gamma is {}\n'.format( gam))
...
# markerline, stemlines, baseline = plt.stem(range(500),x_est[:,111],markerfmt ='*',label='GEN_L1')
...
# plt.legend()
# plt.savefig('Gen_Re_We_l1/figures/Gen_L1 with {} sparse and measurement {}.png'.format(args.sparsity, m))
# plt.show()
