...
def forward(y, x_test, thr_, d, B, W, numIter, p, J, tau, eps):
    x           = []
    x.append(d)
    # RSNR_iter = []
    ...
    for iter in range(numIter):
      ...
      # weights = tau*k/(np.abs(d)**p + eps)**(1/(k+1))
...
y_test    = data_gen(D, snr, x_test, rng)
...
# f = open("Gen_Re_lp/logs2/Gen_LP2 measurement {} sparsity {}.txt".format(m, args.sparsity), "w")
# # f.write('Lamda is {}\n'.format( lam))
# # f.write('gamma is {}\n'.format( gam))
